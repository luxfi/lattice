...
func (cmb Combiner) GenAdditiveShare(activesPoints []ShamirPublicPoint, ownPoint ShamirPublicPoint, ownShare ShamirSecretShare, skOut *rlwe.SecretKey) (err error) {
	...
	// Validate that every active party (other than self) has a precomputed Lagrange
	// coefficient. Without this check, a stale or churned active-set would silently
	// pass `nil` into MulRNSScalar and panic deep in scalar arithmetic.
	for _, active := range activesPoints[:cmb.threshold] {
		if active == ownPoint {
			continue
		}
		if _, ok := cmb.lagrangeCoeffs[active]; !ok {
			return fmt.Errorf("cannot GenAdditiveShare: active party %d not in the Combiner's others set", active)
		}
	}
...
// BinarySize returns the serialized size of the object in bytes.
func (s ShamirPolynomial) BinarySize() int {
...
// interface, and will write exactly object.BinarySize() bytes on w.
//
// Security: a [ShamirPolynomial] holds the generator polynomial whose constant
// term is the dealer's secret. The marshalled bytes MUST be encrypted at rest
// and MUST NOT be transmitted over a non-private channel.
//
// Unless w implements the [buffer.Writer] interface (see lattice/utils/buffer/writer.go),
// it will be wrapped into a [bufio.Writer]. Since this requires allocations, it
// is preferable to pass a [buffer.Writer] directly:
...
//   - When writing to a pre-allocated var b []byte, it is preferable to pass
//     buffer.NewBuffer(b) as w (see lattice/utils/buffer/buffer.go).
func (s ShamirPolynomial) WriteTo(w io.Writer) (n int64, err error) {
...
//   - When reading from a var b []byte, it is preferable to pass a buffer.NewBuffer(b)
//     as w (see lattice/utils/buffer/buffer.go).
func (s *ShamirPolynomial) ReadFrom(r io.Reader) (n int64, err error) {
	return s.Value.ReadFrom(r)
}

// MarshalBinary encodes the object into a binary form on a newly allocated slice
// of bytes. See [ShamirPolynomial.WriteTo] for the security caveats — this
// output reveals the dealer's secret and MUST be protected accordingly.
func (s ShamirPolynomial) MarshalBinary() (p []byte, err error) {
	return s.Value.MarshalBinary()
}

// UnmarshalBinary decodes a slice of bytes generated by
// [ShamirPolynomial.MarshalBinary] or [ShamirPolynomial.WriteTo] on the object.
func (s *ShamirPolynomial) UnmarshalBinary(p []byte) (err error) {
